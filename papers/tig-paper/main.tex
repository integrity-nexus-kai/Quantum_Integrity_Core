...
The numerical behavior is illustrated in Fig.~1.


\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{tig_photon_sphere_deviation.png}
\caption{Relative deviation of the photon sphere radius in TIG compared to General Relativity.}
\end{figure}
---
...
To assess the robustness of the bifurcation structure, we consider a generalized class of regular mass profiles of the form
\begin{equation}
m(r) = M \frac{r^n}{r^n + r_c^n}, \quad n \geq 3.
\end{equation}

This class satisfies the generic conditions expected for regular black hole solutions:
\begin{itemize}
\item $m(r) \to M$ as $r \to \infty$,
\item $m(r) \sim r^n$ as $r \to 0$.
\end{itemize}

The corresponding metric function is
\begin{equation}
F(r) = 1 - \frac{2M}{r} \frac{r^n}{r^n + r_c^n}.
\end{equation}
...
This condition implies the existence of a **double root**, independently of the specific value of $n$.

\subsection{Universality of the Local Structure}

Near criticality, any smooth function $F(r)$ with a double root admits a Taylor expansion:
\begin{equation}
...
The echo delay is given by
\begin{equation}
\Delta t_{\text{echo}} \sim \int \frac{dr}{F(r)}.
\end{equation}

Using the quadratic approximation, we obtain
\begin{equation}
\Delta t_{\text{echo}} \sim (\beta_c - \beta)^{-1/2}.
\end{equation}

This scaling is therefore **universal** and independent of:
\begin{itemize}
\item the exponent $n$,
\item the detailed functional form of $m(r)$,
\item the global geometry.
\end{itemize}

\subsection{Conclusion of the Appendix}

The existence of a bifurcation, its classification as a saddle-node transition, and the associated critical scaling behavior follow solely from:
\begin{itemize}
\item the presence of a regularization scale $r_c$,
\item the existence of a double root in the horizon equation.
\end{itemize}

These features are therefore structurally stable and do not depend on the specific effective ansatz used in the main text.
